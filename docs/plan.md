...
- ✅ **4e coéquipier : modèle ML (xG)** — `HistGradientBoostingClassifier` (scikit-learn) sur features enrichies **xG glissant** (expected goals créé/concédé), Elo et forme. Le xG capte la qualité de jeu indépendante du résultat → signal neuf que les autres modèles n'ont pas. Calibré (isotonique), entraîné par ligue, ajouté au consensus avec poids 0.22. **Validé EPL 2023+2024 (walk-forward) : log-loss 0.995 vs 1.006 pour l'Elo seul.** Modules `ml.py` + `ml_service.py` ; entraînement via `/api/ml/train?league=&saisons=` (1 appel API/match la 1re fois, puis cache). Source optionnelle : si la ligue n'est pas entraînée, le consensus l'ignore proprement
- ✅ **Poids du consensus optimisés** — balayage sur 1669 matchs walk-forward (toutes ligues club en cache) minimisant le log-loss. L'Elo s'est révélé plus fiable sur l'ensemble (Elo seul 1.031 vs Poisson seul 1.122). Nouveaux poids : sans marché **Elo 0.70 / Poisson 0.30** (log-loss 1.018 vs 1.029 avant) ; avec marché **Marché 0.50 / Elo 0.30 / Poisson 0.20**. Compromis volontaire vers le Poisson (mieux nourri en prod : ~2 saisons ajustées vs demi-saison au backtest)
