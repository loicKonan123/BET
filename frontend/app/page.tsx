    ...
    <>
      ...
      <div className="mb-xl max-w-4xl">
        <h1 className="font-headline-lg text-headline-lg text-primary mb-xs">
          BET — Générateur de tickets
        </h1>
        <p className="font-body-lg text-on-surface-variant">
          Algorithmes de prédiction haute fidélité basés sur l&apos;analyse de
      ...
      {res && res.combines.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-lg mb-xl">
          {res.combines.map((c, i) => (
            <div
              key={i}
              className="glass-card p-lg rounded-xl relative overflow-hidden"
            >
                  ...
                  <div className="flex items-center gap-xs">
                    <div className="h-1 w-16 bg-surface-container-highest rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${c.proba_reussite * 100}%` }}
                      />
                    </div>
              ...
              <div className="flex flex-col gap-sm mb-lg">
                {c.selections.map((s, j) => (
                  <div
                    key={j}
                    className="flex items-center justify-between p-sm rounded bg-white/5 border border-white/5"
                  >
                    <div className="flex flex-col">
              ...
              <div className="grid grid-cols-3 gap-sm pt-md border-t border-white/10">
                ...
                <Stat
                  label="Proba"
                  value={`${Math.round(c.proba_reussite * 100)}%`}
                  color="secondary"
                />
                <Stat
                  label="Value"
                  value={`${c.value >= 0 ? "+" : ""}${Math.round(c.value * 100)}%`}
                  color="tertiary"
                />
              </div>
      ...
      {/* Analyse des matchs (stats équipes) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-md">
            {res.analyses.map((a) => (
              <div key={a.fixture_id} className="glass-card p-md rounded-lg">
                ...
                <div className="font-body-md text-body-md text-on-surface mb-sm">
                  {a.match}
                </div>
                <div className="flex justify-between font-label-sm text-label-sm">
                  <span className="text-on-surface-variant">
                    ...
                    <span className="text-primary">
                      {a.buts_attendus.domicile} - {a.buts_attendus.exterieur}
                    </span>
                  </span>
                  <span className="text-on-surface-variant">
                    Forme{" "}
                    <span className="text-secondary">
                      {a.forme.domicile}/{a.forme.exterieur}
                    </span>
                  </span>
                </div>
              </div>
...
function Stat({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span
        className={`${cls} px-sm py-xs rounded font-label-md text-label-md font-bold w-full text-center`}
      >
        {value}
      </span>
    </div>
